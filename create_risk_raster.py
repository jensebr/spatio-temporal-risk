...
#pysmile.License((
    # ! 
    # INSERT YOUR PYSMILE LICENCE HERE
    # !
	#))
...
def create_risk_raster(altitude_ev, case):
    ...
    # Open raster layers
    current_speed_ds = gdal.Open("/GIS_files/current_filtered.tif")
    slope_ds = gdal.Open("/GIS_files/slope_filtered_s10_r20.tif")
    ruggedness_ds = gdal.Open("/GIS_files/ruggedness_filtered.tif")
    bpi_broad_ds = gdal.Open("/GIS_files/bpi_broad_reduced.tif")
    depth_ds = gdal.Open("/GIS_files/depth_reduced.tif")
    coral_ds = gdal.Open("/GIS_files/corals_filtered_358_374.tif")
    ...
    risk_map_ds = driver_tiff.Create("/GIS_files/risk_map_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h1_ds  = driver_tiff.Create("/GIS_files/h1_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h2_ds  = driver_tiff.Create("/GIS_files/h2_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h3_ds  = driver_tiff.Create("/GIS_files/h3_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h4_ds  = driver_tiff.Create("/GIS_files/h4_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h5_ds  = driver_tiff.Create("/GIS_files/h5_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h6_ds  = driver_tiff.Create("/GIS_files/h6_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h7_ds  = driver_tiff.Create("/GIS_files/h7_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    h8_ds  = driver_tiff.Create("/GIS_files/h8_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    a1_ds  = driver_tiff.Create("/GIS_files/a1_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    a2_ds  = driver_tiff.Create("/GIS_files/a2_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    a3_ds  = driver_tiff.Create("/GIS_files/a3_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    a4_ds  = driver_tiff.Create("/GIS_files/a4_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    a5_ds  = driver_tiff.Create("/GIS_files/a5_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    rec_ds = driver_tiff.Create("/GIS_files/rec_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    cata_ds = driver_tiff.Create("/GIS_files/cata_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    crit_ds = driver_tiff.Create("/GIS_files/crit_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    mod_ds = driver_tiff.Create("/GIS_files/mod_" + name_postfix, xsize=depth_ds.RasterXSize, ysize=depth_ds.RasterYSize, bands=1, eType=gdal.GDT_Float32)
    ...
    # Get size of rasters, used for looping through arrays
    ...
    bn = pysmile.Network()
    bn.read_file("spatio-temporal_0512_dev.xdsl")
    for handle in bn.get_all_nodes():
        if bn.get_node_id(handle) == "Risk":
            risk_handle = handle
            break

    # Start looping through every cell in rasters
    num_iter = cols*rows
    it = 1
    for col in range(cols):
        ...
        for row in range(rows):
            ...
            # Set NoValue (NaN) if cell is invalid
            if coral > 1.0 or coral < 0.0 or slope < -1:
                ...
                # Retrive probabilities for hazards
                ...
                # Retrieve probabilities for hazardous events
                ...
                consequence_min_gate_distribution = bn.get_node_value("samlenode")

                # Retrieve expected value risk metric
                risk_value = bn.get_node_value("Total_risk")
                #risk_map_array[row][col] = risk_value[0]

                # Store values in rasters
                ...
                cata_array[row][col] = math.log10(consequence_min_gate_distribution[0])
                crit_array[row][col] = math.log10(consequence_min_gate_distribution[1])
                mod_array[row][col] = math.log10(consequence_min_gate_distribution[2])

                alpha = 1/3
                cost_vec = [3000000, 8000, 800, 400]

                # Calculates the conditional variance at risk metric by interpolation
                c1_dist = bn.get_node_value("C1")
                if c1_dist[0] > (1-alpha):
                    c1_cvar = cost_vec[0]*c1_dist[4]/alpha + cost_vec[1]*c1_dist[3]/alpha + cost_vec[2]*c1_dist[2]/alpha + cost_vec[3]*c1_dist[1]/alpha
                elif (c1_dist[0] + c1_dist[1]) > (1-alpha):
                    rem = (c1_dist[0] + c1_dist[1]) - (1-alpha)
                    c1_cvar = cost_vec[0]*c1_dist[4]/alpha + cost_vec[1]*c1_dist[3]/alpha + cost_vec[2]*c1_dist[2]/alpha + cost_vec[3]*rem/alpha
                elif (c1_dist[0] + c1_dist[1] + c1_dist[2]) > (1-alpha):
                    rem = (c1_dist[0] + c1_dist[1] + c1_dist[2]) - (1-alpha)
                    c1_cvar = cost_vec[0]*c1_dist[4]/alpha + cost_vec[1]*c1_dist[3]/alpha + cost_vec[2]*rem/alpha

                c2_dist = bn.get_node_value("C2")
                if c2_dist[0] > (1-alpha):
                    c2_cvar = cost_vec[0]*c2_dist[4]/alpha + cost_vec[1]*c2_dist[3]/alpha + cost_vec[2]*c2_dist[2]/alpha + cost_vec[3]*c2_dist[1]/alpha
                elif (c2_dist[0] + c2_dist[1]) > (1-alpha):
                    rem = (c2_dist[0] + c2_dist[1]) - (1-alpha)
                    c2_cvar = cost_vec[0]*c2_dist[4]/alpha + cost_vec[1]*c2_dist[3]/alpha + cost_vec[2]*c2_dist[2]/alpha + cost_vec[3]*rem/alpha
                elif (c2_dist[0] + c2_dist[1] + c2_dist[2]) > (1-alpha):
                    rem = (c2_dist[0] + c2_dist[1] + c2_dist[2]) - (1-alpha)
                    c2_cvar = cost_vec[0]*c2_dist[4]/alpha + cost_vec[1]*c2_dist[3]/alpha + cost_vec[2]*rem/alpha

                c3_dist = bn.get_node_value("C3")
                if c3_dist[0] > (1-alpha):
                    c3_cvar = cost_vec[0]*c3_dist[4]/alpha + cost_vec[1]*c3_dist[3]/alpha + cost_vec[2]*c3_dist[2]/alpha + cost_vec[3]*c3_dist[1]/alpha
                elif (c3_dist[0] + c3_dist[1]) > (1-alpha):
                    rem = (c3_dist[0] + c3_dist[1]) - (1-alpha)
                    c3_cvar = cost_vec[0]*c3_dist[4]/alpha + cost_vec[1]*c3_dist[3]/alpha + cost_vec[2]*c3_dist[2]/alpha + cost_vec[3]*rem/alpha
                elif (c3_dist[0] + c3_dist[1] + c3_dist[2]) > (1-alpha):
                    rem = (c3_dist[0] + c3_dist[1] + c3_dist[2]) - (1-alpha)
                    c3_cvar = cost_vec[0]*c3_dist[4]/alpha + cost_vec[1]*c3_dist[3]/alpha + cost_vec[2]*rem/alpha

                c4_dist = bn.get_node_value("C4")
                if c4_dist[0] > (1-alpha):
                    c4_cvar = cost_vec[0]*c4_dist[4]/alpha + cost_vec[1]*c4_dist[3]/alpha + cost_vec[2]*c4_dist[2]/alpha + cost_vec[3]*c4_dist[1]/alpha
                elif (c4_dist[0] + c4_dist[1]) > (1-alpha):
                    rem = (c4_dist[0] + c4_dist[1]) - (1-alpha)
                    c4_cvar = cost_vec[0]*c4_dist[4]/alpha + cost_vec[1]*c4_dist[3]/alpha + cost_vec[2]*c4_dist[2]/alpha + cost_vec[3]*rem/alpha
                elif (c4_dist[0] + c4_dist[1] + c4_dist[2]) > (1-alpha):
                    rem = (c4_dist[0] + c4_dist[1] + c4_dist[2]) - (1-alpha)
                    c4_cvar = cost_vec[0]*c4_dist[4]/alpha + cost_vec[1]*c4_dist[3]/alpha + cost_vec[2]*rem/alpha

                c5_dist = bn.get_node_value("C5")
                if c5_dist[0] > (1-alpha):
                    c5_cvar = cost_vec[0]*c5_dist[4]/alpha + cost_vec[1]*c5_dist[3]/alpha + cost_vec[2]*c5_dist[2]/alpha + cost_vec[3]*c5_dist[1]/alpha
                elif (c5_dist[0] + c5_dist[1]) > (1-alpha):
                    rem = (c5_dist[0] + c5_dist[1]) - (1-alpha)
                    c5_cvar = cost_vec[0]*c5_dist[4]/alpha + cost_vec[1]*c5_dist[3]/alpha + cost_vec[2]*c5_dist[2]/alpha + cost_vec[3]*rem/alpha
                elif (c5_dist[0] + c5_dist[1] + c5_dist[2]) > (1-alpha):
                    rem = (c5_dist[0] + c5_dist[1] + c5_dist[2]) - (1-alpha)
                    c5_cvar = cost_vec[0]*c5_dist[4]/alpha + cost_vec[1]*c5_dist[3]/alpha + cost_vec[2]*rem/alpha
